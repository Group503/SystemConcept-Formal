...
public class DevicePanel extends JPanel {
    ...
    // 总设备类表
    private DefaultTableModel tableModel_Device = new DefaultTableModel(null, new String[]{}) {
    ...
    // 设备状态表
    private DefaultTableModel tableModel_DeviceStatus = new DefaultTableModel(null, new String[]{}) {
        //内部类重写isCellEditable方法，设置JTable不可编辑
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
    ...
    // 进程占用设备表
    private DefaultTableModel tableModel_ProcessInfo = new DefaultTableModel(null, new String[]{}) {
        //内部类重写isCellEditable方法，设置JTable不可编辑
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
    ...
    // 设备等待队列表
    private DefaultTableModel tableModel_Queue = new DefaultTableModel(null, new String[]{}) {
        //内部类重写isCellEditable方法，设置JTable不可编辑
        @Override
        public boolean isCellEditable(int row, int column) {
            return false;
    ...
    /**
     * 有参构造方法
     * @param allDevice 总设备类表
     * @param allDeviceStatus 设备状态表
     * @param processInfo 进程占用设备信息表
     * @param d_Queue 设备等待队列
     */
    DevicePanel(Map<String, Device> allDevice, ArrayList<DeviceStatus> allDeviceStatus, Map<Integer, DeviceInfoMap> processInfo, Queue<QueueElem> d_Queue) {
 ...
 // ***************************************************************以上为更新Queue_Text
    ...
    /**
     * 更新总设备类表
     * @param allDevice 设备类表
     */
    private void updateDevice(Map<String, Device> allDevice) {
        Vector columnNames = new Vector(Arrays.asList(new String[]{"设备名", "总设备数", "空闲设备数", "相对地址"}));//列名
        Vector allRowVector = new Vector();//JTable总数据
    ...
    private void updateDeviceStatus(ArrayList<DeviceStatus> allDeviceStatus) {
        Vector columnNames = new Vector(Arrays.asList(new String[]{"设备名", "设备ID", "是否分配", "占用进程ID", "相对地址号"}));//列名
        Vector allRowVector = new Vector();//JTable总数据

        for (int i = 0;
                i < allDeviceStatus.size();
                i++) {
            Vector columnDate = new Vector();//一行的列数据
    ...
    /**
     * 更新进程占用设备表
     * @param processInfo 进程占用设备信息
     */
    private void updateProcessInfo(Map<Integer, DeviceInfoMap> processInfo) {
        Vector columnNames = new Vector(Arrays.asList(new String[]{"进程ID", "A", "B", "C"}));//列名
        Vector allRowVector = new Vector();//JTable总数据
