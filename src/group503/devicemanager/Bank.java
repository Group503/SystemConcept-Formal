...
public class Bank {
    ...
    private static int isSafe = -999;// 系统状态，0不存在安全系列，1存在安全系列且能立即满足序列首==当前进程的申请，2存在安全序列但当前进程需要等待
    // 初始-999目的是作为递归循环继续的标志
     ...
     * 全排列，递归
     *
     * @param list 全排列因子，此处为进程ID
     * @param k
     * @param m
     * @return -999/0/1/2
     */
    private int Perm(Integer list[], int k, int m) {
        if (k == m) {
            ...
            for (i = 0; i <= m; i++) {
                ...
                if (!isLegal) {
                    ...
                    this.bkProcessES.get(curPc_ID).Finished = true;
            ...
            if (i > m) {// 存在安全系列，可以返回了isAllFinished()
     ...
     */
    public int Safety() {
        // 1.初始化Avaliable和Work，进程集合bkProcess，即将数据填充过来
        ...
        invalidOrder.clear();//初始化
        isSafe = -999;// 初始化，-999为递归继续的标志
        int status = Perm(list, 0, list.length - 1);//可能返回-999，0，1，2
